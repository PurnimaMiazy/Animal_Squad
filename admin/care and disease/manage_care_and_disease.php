...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `care_and_disease` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title"><?php echo isset($id) ? "Update ": "Create New " ?> Care and Disease</h3>
    </div>
    <div class="card-body">
        <form action="" id="product-form">
            ...
            <div class="form-group">
                ...
                ?>
                <option value="<?php echo $row['id'] ?>" <?php echo isset($category_id) && $category_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['care_category'] ?></option>
                <?php endwhile; ?>
                </select>
            </div>
            <!-- Sub Category -->
            <div class="form-group">
                ...
                <?php
                    $qry = $conn->query("SELECT * FROM `care_sub_categories` order by care_sub_category asc");
                    $care_sub_categories = array();
                    while($row= $qry->fetch_assoc()):
                    $care_sub_categories[$row['care_parent_id']][] = $row;
                    endwhile; 
                ?>
                </select>
            </div>
            <!-- Name -->
            <div class="form-group">
                <label for="care_and_disease_name" class="control-label">Care and Disease Name</label>
                <textarea name="care_and_disease_name" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($care_and_disease_name) ? $care_and_disease_name : ''; ?></textarea>
            </div>
            <!-- Description -->
            <div class="form-group">
    ...
    <div class="card-footer">
        ...
        <a class="btn btn-flat btn-default" href="?page=care and disease/index">Cancel</a>
    </div>
</div>
<script>
    ...
    var sub_categories = $.parseJSON('<?php echo json_encode($care_sub_categories) ?>');
    $(document).ready(function(){
                ...
                if(k == cid){
                    Object.keys(sub_categories[k]).map(i=>{
                        if('<?php echo isset($sub_category_id) ? $sub_category_id : 0 ?>' == sub_categories[k][i].id){
                            opt += "<option value='"+sub_categories[k][i].id+"' selected>"+sub_categories[k][i].care_sub_category+"</option>";
                        }else{
                            opt += "<option value='"+sub_categories[k][i].id+"'>"+sub_categories[k][i].care_sub_category+"</option>";
                        }
                    })
                }
            })
        ...
        $('#product-form').submit(function(e){
            ...
            start_loader();
            $.ajax({
                url:_base_url_+"classes/Master.php?f=save_care_and_disease",
                ...
                error:err=>{
                    console.log(err)
                    alert_toast("An errors occured",'error');
                    end_loader();
                },
                success:function(resp){
                    if(typeof resp =='object' && resp.status == 'success'){
                        location.href = "./?page=care and disease/index";
