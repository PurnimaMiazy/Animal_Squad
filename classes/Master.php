...
Class Master extends DBConnection {
	...
	function capture_err(){
		...
		function save_care_sub_category(){
		extract($_POST);
		$data = "";
		...
		$check = $this->conn->query("SELECT * FROM `care_sub_categories` where `care_sub_category` = '{$care_sub_category}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
		...
		if(empty($id)){
			$sql = "INSERT INTO `care_sub_categories` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `care_sub_categories` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	function save_product(){
		...
		if($save){
			$upload_path = "uploads/adopts_".$id;
...
function save_care_category(){
		...
		$check = $this->conn->query("SELECT * FROM `care_categories` where `care_category` = '{$care_category}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `care_categories` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `care_categories` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	function delete_care_category(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `care_categories` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Category successfully deleted.");
		}else{
	...
	function save_care_and_disease(){
		extract($_POST);
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id','description'))){
		...
		$check = $this->conn->query("SELECT * FROM `care_and_disease` where `care_and_disease_name` = '{$care_and_disease_name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `care_and_disease` set {$data} ";
			$save = $this->conn->query($sql);
			$id= $this->conn->insert_id;
		}else{
			$sql = "UPDATE `care_and_disease` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$upload_path = "uploads/care_".$id;
			if(!is_dir(base_app.$upload_path))
				mkdir(base_app.$upload_path);
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				foreach($_FILES['img']['tmp_name'] as $k => $v){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Information successfully saved.");
			else
	...
	function delete_care_sub_category(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `care_sub_categories` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Sub Category successfully deleted.");
		}else{
...
function save_care_and_disease_product(){
		extract($_POST);
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id','description'))){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Information already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Information successfully saved.");
			else
				$this->settings->set_flashdata('success',"Information successfully updated.");
		}else{
			$resp['status'] = 'failed';
	...
	function delete_care_and_disease(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `care_and_disease` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function save_inventory(){
		...
		$check = $this->conn->query("SELECT * FROM `inventory` where `product_id` = '{$product_id}' and `size` = 'M' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
...
switch ($action) {
	...
	break;
        case 'save_care_category':
		echo $Master->save_care_category();
	break;
	case 'delete_care_category':
		echo $Master->delete_care_category();
	break;
        case 'save_care_sub_category':
		echo $Master->save_care_sub_category();
	break;
        case 'delete_care_sub_category':
		echo $Master->delete_care_sub_category();
	break;
	case 'save_care_and_disease':
		echo $Master->save_care_and_disease();
	break;
	case 'delete_care_and_disease':
		echo $Master->delete_care_and_disease();
	break;
	case 'save_inventory':
