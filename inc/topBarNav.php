<nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container px-4 px-lg-5 ">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-4">
                        ...
                        <?php endwhile; ?>
                        <?php 
                        $care_cat_qry = $conn->query("SELECT * FROM care_categories where status = 1 ");
                        while($ccrow = $care_cat_qry->fetch_assoc()):
                          $care_sub_qry = $conn->query("SELECT * FROM care_sub_categories where status = 1 and care_parent_id = '{$ccrow['id']}' ");
                          if($care_sub_qry->num_rows <= 0):
                        ?>
                        <li class="nav-item"><a class="nav-link" aria-current="page" href="./?p=care_and_disease&c=<?php echo md5($ccrow['id']) ?>"><?php echo $ccrow['care_category'] ?></a></li>
                        
                        <?php else: ?>
                        <li class="nav-item dropdown">
                          <a class="nav-link dropdown-toggle" id="navbarDropdown<?php echo $ccrow['id'] ?>" href="#" role="button" data-toggle="dropdown" aria-expanded="false"><?php echo $ccrow['care_category'] ?></a></a>
                            <ul class="dropdown-menu  p-0" aria-labelledby="navbarDropdown<?php echo $ccrow['id'] ?>">
                              <?php while($csrow = $care_sub_qry->fetch_assoc()): ?>
                                <li><a class="dropdown-item border-bottom" href="./?p=care_and_disease&c=<?php echo md5($ccrow['id']) ?>&s=<?php echo md5($csrow['id']) ?>"><?php echo $csrow['care_sub_category'] ?></a></li>
                            <?php endwhile; ?>
                            </ul>
                        </li>
                        <?php endif; ?>
			...
			<li class="nav-item"><a class="nav-link" href="./?p=share_image">Share Image</a></li>
			<li class="nav-item"><a class="nav-link" href="./?p=storyshare">Story</a></li>
