<?php 
 ...
 if($products->num_rows > 0){
    ...
    $upload_path = base_app.'/uploads/adopts_'.$id;
    $img = "";
    if(is_dir($upload_path)){
        $fileO = scandir($upload_path);
        if(isset($fileO[2]))
            $img = "uploads/adopts_".$id."/".$fileO[2];
        // var_dump($fileO);
    }
                    ...
                    <?php 
                        foreach($fileO as $k => $img):
                            if(in_array($img,array('.','..')))
                                continue;
                    ?>
                        <a href="javascript:void(0)" class="view-image <?php echo $k == 2 ? "active":'' ?>"><img src="<?php echo validate_image('uploads/adopts_'.$id.'/'.$img) ?>" loading="lazy"  class="img-thumbnail" alt=""></a>
                    <?php endforeach; ?>
                </div>
            </div>
            <div class="col-md-6">
                ...
                <div class="fs-5 mb-5">
                TK: <span id="price"><?php echo $inv[0]['price'] ?></span>
                <br>
